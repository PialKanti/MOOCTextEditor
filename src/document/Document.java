...
public abstract class Document {
	...
	protected int countSyllables(String word) {
		...
		int NumSyllables = 0;
		boolean syllableStart;
		String vowels = "aeiouy";

		syllableStart = true;
		for (int i = 0; i < word.length(); i++) {
			if (i == word.length() - 1 && Character.toLowerCase(word.charAt(i)) == 'e' && syllableStart
					&& NumSyllables > 0) {
				NumSyllables--;
			}
			if (syllableStart && vowels.indexOf(Character.toLowerCase(word.charAt(i))) >= 0) {
				syllableStart = false;
				NumSyllables++;
	...
	public static boolean testCase(Document doc, int syllables, int words, int sentences) {
		...
		System.out.println(syllFound + " " + wordsFound + " " + sentFound);
		return passed;
	}
